"use client"
import { ArrowArcLeft, CaretCircleLeft, CaretCircleRight, ShieldChevron, TagChevron } from "@phosphor-icons/react";
...
const Carousel: FunctionComponent<CarouselProps> = ({children, slides = [], autoSlide = false, autoSlideInterval = 3000}) => {
    slides = slides!
            ...
            <div className="absolute  bottom-1.5 right-14 ">
                <div className="flex items-center justify-center gap-2">
                    {slides.map((cb: string, index: number)=> (
                        <div key={index} onClick={()=>setCurrent(index)} className={`cursor-pointer transition-all w-2 h-2 bg-color-primary rounded-full
