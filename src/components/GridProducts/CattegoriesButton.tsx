...
const Button = ({ name, activeCat, setActiveCat }: { name: string, activeCat: string, setActiveCat: any }) => {
  ...
  return (
    <button onClick={handleClick} className={`min-h-10 w-auto transition-all duration-500  aspect-video px-2.5  rounded-lg flex justify-center items-center ${name === activeCat ? "bg-color-secondary text-color-primary" : "bg-color-primary border-color-placeholder border text-color-placeholder"}`}>
      {name}
    </button>
  );
};
